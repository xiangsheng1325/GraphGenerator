...
### load datasets
    ...
    # real graphs
        ...
        graphs = Graph_load_batch(min_num_nodes=500, max_num_nodes=1000, name='PROTEINS_full')
        G = max(graphs, key=len)
        max_subg = G.subgraph(max(nx.connected_components(G), key=len))
        G = nx.convert_node_labels_to_integers(max_subg)
    ...
    elif args.graph_type == 'citeseer_e':
        _, _, G = Graph_load(dataset='citeseer')
        G = max(nx.connected_component_subgraphs(G), key=len)
        G = nx.convert_node_labels_to_integers(G)
        graphs = []
        ...
        graphs = graphs[0:200]
        args.max_prev_node = 15
    if args.graph_type in ['cora', 'citeseer', 'pubmed']:
        ...
        graphs = [G]
        args.max_prev_node = 256
    if args.graph_type in ['PROTEINS_full', 'guarantee_loan', 'yeast', 'FIRSTMM_DB'] or args.graph_type.startswith('top'):
        import pickle
        G = pickle.load(open("/home/xiangsheng/venv/ggen/ggen/generators/data/{}.graph".format(args.graph_type), "rb"))
        max_subg = G.subgraph(max(nx.connected_components(G), key=len))
        G = nx.convert_node_labels_to_integers(max_subg)
        G.remove_edges_from(nx.selfloop_edges(G))
        graphs = [G]
        if args.graph_type == 'PROTEINS_full':
            args.max_prev_node = 128
        elif args.graph_type.startswith('top'):
            if int(args.graph_type[3:]) >= 7:
                args.max_prev_node = 128
            else:
                args.max_prev_node = 256
        else:
            args.max_prev_node = 512
    return graphs
...
def load_matlab_graph(fname):
    ...
    #print(tmp_data)
    return tmp_data['G_bter']


def save_matlab_graph(dump_fname, sp_adj, graphname):
...
def mask_adj(adj):
    """
    generate masks of training set and validating set.
