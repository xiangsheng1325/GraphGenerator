...
TVecPool<TVal, TSizeTy>::TVecPool(TSIn& SIn) : FastCopy(SIn) {
  ...
  GrowBy=TSize(_GrowBy);  MxVals=TSize(_Vals);  Vals=TSize(_Vals);
  EmptyVal = TVal(SIn);
...
void TVecPool<TVal, TSizeTy>::CompactPool(const TVal& DelVal) {
  ...
  for (int vid = 1; vid < IdToOffV.Len(); vid++) {
    ...
    if (TotalDel > 0) { IdToOffV[vid-1] -= TotalDel; }
    if (Len == 0) { continue; }
    NDel = 0;
    ...
    memcpy(ValV-TotalDel, ValV, sizeof(TVal)*Len);
    TotalDel += NDel;
  }
