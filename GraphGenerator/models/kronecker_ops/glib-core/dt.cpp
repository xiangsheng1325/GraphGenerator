...
void TStr::SplitOnAllAnyCh(
 const TStr& SplitChStr, TStrV& StrV, const bool& SkipEmpty) const {
  ...
  forever{
    char* BfC=CurStrBf;
    while (*BfC!=0){
      const char* SplitChBfC=SplitChBf;
      while ((*SplitChBfC!=0)&&(*SplitChBfC!=*BfC)){SplitChBfC++;}
      if (*SplitChBfC!=0){break;}
      BfC++;
    }
    bool IsEnd=(*BfC==0);
    *BfC=0;
...
uint TStrPool::AddStr(const char *Str, const uint& Len) {
  IAssertR(Len > 0, "String too short (length includes the null character)");
  if (Len == 1 && BfL > 0) { return 0; }
