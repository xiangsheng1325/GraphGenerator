...
void TTable::UpdateFltFromTableMP(const TStr& KeyAttr, const TStr& UpdateAttr,
  ...
  // : this should be a generic vector operation
  ...
  switch (KeyType) {
    // : add support for other cases of KeyType
    case atInt: {
...
void TTable::UnionAllInPlace(const TTable& Table) {
  AddTable(Table);
  // : For the moment, IDs are not initialized (to avoid having too many ID columns)
  //result->InitIds();
}
