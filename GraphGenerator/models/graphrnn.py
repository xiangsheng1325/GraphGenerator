...
def sample_sigmoid_supervised(y_pred, y, current, y_len, sample_time=2):
    ...
    # do sampling
    y_result = Variable(torch.rand(y_pred.size(0), y_pred.size(1), y_pred.size(2))).to(y.device)
    # loop over all batches
    for i in range(y_result.size(0)):
        # using supervision
        if current<y_len[i]:
            while True:
                y_thresh = Variable(torch.rand(y_pred.size(1), y_pred.size(2))).to(y.device)
...
def message_passing(node_neighbor, node_embedding, model):
    ...
    for i in range(len(node_neighbor)):
        ...
        if neighbor_num > 0:
            ...
            # print("ne: {}".format(node_embedding.shape))
            # node_self_neighbor = torch.cat([node_embedding[j] for j in node_neighbor[i]], dim=0)
            node_self_neighbor = torch.cat([node_embedding[i][:, j] for j in node_neighbor[i]], dim=0)
...
class Graph_RNN_structure(nn.Module):
    ...
    def forward(self, x, teacher_forcing, temperature = 0.5, bptt=True,bptt_len=20, flexible=True,max_prev_node=100):
        ...
        # : current only look back to self.output_size nodes, try to look back according to bfs sequence
...
class GCN_generator(nn.Module):
    def __init__(self, hidden_dim):
        ...
        # : add an linear_input module to map the input feature into 'hidden_dim'
    ...
    def forward(self,x,teacher_force=False,adj_real=None):
        ...
        # do GCN n times
        # : try if residual connections are plausible
        # : add higher order of adj (adj^2, adj^3, ...)
        # : try if norm everytim is plausible
